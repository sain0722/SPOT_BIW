...
class SpotCameraParameter:
    ...
    def get_resolution(self):
        ...
        resolution_code = params.camera_mode
        s_resolution = ""
        if resolution_code == 1:
            s_resolution = "1280x720"
        elif resolution_code == 11:
            s_resolution = "640x480"
        elif resolution_code == 14:
            s_resolution = "1920x1080"
        elif resolution_code == 15:
            s_resolution = "3840x2160"
        elif resolution_code == 16:
            s_resolution = "4208x3120"
        elif resolution_code == 17:
            s_resolution = "4096x2160"
    ...
    def set_hdr(self, s_hdr_mode, b_flag=True):
        """
        :@param
            s_hdr_mode:
    ...
    def set_led_mode(self, s_led_mode, b_flag=True):
        gn_led_mode = None
        if s_led_mode == 'OFF':
        ...
        elif s_led_mode == 'FLASH':
            gn_led_mode = gripper_camera_param_pb2.GripperCameraParams.LED_MODE_FLASH
        elif s_led_mode == 'BOTH':
            gn_led_mode = gripper_camera_param_pb2.GripperCameraParams.LED_MODE_BOTH
    ...
    def set_led_torch_brightness(self, f_torch_brightness, b_flag=True):
        if self.get_gripper_params().led_mode:
            return None

        gf_led_torch_brightness = wrappers_pb2.FloatValue(value=f_torch_brightness)
        if b_flag:
            params = gripper_camera_param_pb2.GripperCameraParams(led_torch_brightness=gf_led_torch_brightness)
            request = gripper_camera_param_pb2.GripperCameraParamRequest(params=params)
            response = self.gripper_camera_param_client.set_camera_params(request)
            return response
        else:
            return gf_led_torch_brightness
