...
"""Graph nav utility ui_functions"""
...
def pretty_str_waypoints(waypoint_id, waypoint_name, short_code_to_count, localization_id):
    short_code = id_to_short_code(waypoint_id)
    if short_code is None or short_code_to_count[short_code] != 1:
        short_code = '  '  # If the short code is not valid/unique, don't show it.

    string = f"%s Waypoint name: %s id: %s short code: %s" % \
             ('→' if localization_id == waypoint_id else '  ', waypoint_name, waypoint_id, short_code)
...
def update_waypoints_and_edges(graph, localization_id, do_print=True):
    ...
    waypoints_str_list = []
    if do_print:
        len_waypoint_str = f'%d waypoints:' % len(graph.waypoints)
        # print(len_waypoint_str)
        waypoints_str_list.append(len_waypoint_str)
        for waypoint in waypoint_to_timestamp:
            # pretty_print_waypoints(waypoint[0], waypoint[2], short_code_to_count, localization_id)
            st = pretty_str_waypoints(waypoint[0], waypoint[2], short_code_to_count, localization_id)
            waypoints_str_list.append(st)
    ...
    for edge in graph.edges:
        ...
        if do_print:
            edge_str = "(Edge) from waypoint {} to waypoint {} (cost {})".format(
                       edge.id.from_waypoint, edge.id.to_waypoint, edge.annotations.cost.value)
            # print(edge_str)
            edge_str_list.append(edge_str)

    return name_to_id, edges, waypoints_str_list, edge_str_list
